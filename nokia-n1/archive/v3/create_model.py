...
for side in (-1, 1):
    for z in (3.7, 150.3):
        union(shell, cyl('Cradle fixing boss', 3.5, 7.7, (side*96, 6.95, z)))
        boolean(shell, cyl('Cradle M3 pilot', 1.35, 7, (side*96, 7.5, z)))
    for z in (17, 137):
...
for side in (-1, 1):
    for z in (3.7, 150.3):
        ...
        boolean(cradle, cyl('Facade boss clearance', 3.9, 8, (side*96, 7.0, z)))
        boolean(cradle, cyl('Cradle through screw', 1.7, 20, (side*96, 12, z)))
    boolean(cradle, cyl('Keeper pilot', 1.35, 7, (side*65, 14.7, 149)))
...
for side in (-1, 1):
    ...
    obj['print_part'] = True
    obj = panel(f'05_Top_keeper_{side}', 12, 16, 2.6, (side*65, 19.1, 144.5), 1.5, ivory)
    boolean(obj, cyl('M3 clearance', 1.7, 5, (side*65, 19.1, 149)))
    union(obj, box('Keeper contact toe', (10, 6.7, 5), (side*65, 14.65, 140.5)))
    obj['print_part'] = True
...
for side in (-1, 1):
    for z in (3.7, 150.3):
        cyl('Hardware • M3 cradle screw', 2.7, 1.8, (side*96, 18.5, z), silver, vertices=32)
    for z in (17, 137):
        cyl('Hardware • M3 rear cover screw', 2.7, 1.8, (side*150, 14.5, z), silver, vertices=32)
    s = cyl('Hardware • M3 keeper screw', 2.7, 1.8, (side*65, 21.4, 149), silver, vertices=32)
    boolean(s, box('Screw slot', (3.8, 1.1, 0.65), (side*65, 22.2, 149)))
...
scene['model_revision'] = 'N1 Reference V3'
